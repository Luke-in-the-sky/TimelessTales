...
from llm_api import LargeLanguageModelAPI, SupportedLLMTypes, LargeLanguageModelAPIError, prompt_templates

@pytest.fixture
def mock_transformers():
    with patch('llm_api.pipeline') as mock_pipeline, \
         patch('llm_api.AutoTokenizer.from_pretrained') as mock_tokenizer, \
         patch('llm_api.AutoModelForCausalLM.from_pretrained') as mock_model, \
         patch.dict(prompt_templates, {"dummy_model": "{message}", "new_dummy_model": "{system_message} {message}"}):
...
def test_set_model_id(mock_transformers):
    ...
    api = LargeLanguageModelAPI(model_type=SupportedLLMTypes.HF_CAUSAL, model_id="dummy_model")
...
def test_compose_prompt(mock_transformers):
    api = LargeLanguageModelAPI(model_type=SupportedLLMTypes.HF_SEQ2SEQ, model_id="dummy_model")
    prompt = api.compose_prompt(message="Hello World")
    assert prompt == "Hello World"

    api = LargeLanguageModelAPI(model_type=SupportedLLMTypes.HF_SEQ2SEQ, model_id="new_dummy_model")
    prompt = api.compose_prompt(message="Hello World", system_message="System Message")
    assert prompt == "System Message Hello World"

    with pytest.raises(ValueError):
        api = LargeLanguageModelAPI(model_type=SupportedLLMTypes.HF_SEQ2SEQ, model_id="unknown_model")
